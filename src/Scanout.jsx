...
const Scanout = () => {
  ...
  const { updateAttendeeData, increaseAvailableSeats } = useContext(GlobalStateContext);
      ...
      if (refId !== '') {
        ...
        try {
          ...
          if (response.data.records.length > 0) {
            ...
            increaseAvailableSeats(); // Increase available seats count
          } else {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-amber-500 to-amber-700 flex items-center justify-center">
      <div className="bg-white p-8 rounded-lg shadow-2xl transform hover:scale-105 transition-transform duration-300">
        <h1 className="text-5xl font-extrabold mb-6 text-center text-transparent bg-clip-text bg-gradient-to-r from-amber-600 to-amber-800">
          Scan Out ID
        </h1>
        <div className="mb-6">
          <label
            htmlFor="roomSelect"
            className="block text-xl font-semibold text-amber-700"
          >
            Select Room:
          </label>
          <select
            ...
            className="mt-2 block w-full py-3 px-4 border-2 border-amber-500 bg-white rounded-md shadow-md focus:outline-none focus:ring-4 focus:ring-amber-300 focus:border-transparent text-xl text-gray-800"
          >
        ...
        <input
          ...
          className="w-full px-6 py-4 rounded-lg border-4 border-amber-500 focus:outline-none focus:ring-4 focus:ring-amber-300 focus:border-transparent text-2xl text-gray-800 placeholder-gray-400"
        />
        {loading && (
          <p className="mt-6 text-2xl text-center text-amber-600 animate-pulse">
            Loading...
          </p>
        )}
        {error && (
