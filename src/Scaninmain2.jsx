  ...
  const handleKeyPress = async (e) => {
    if (e.key === 'Enter') {
      ...
      if (refId !== '') {
        ...
        try {
          const [attendeeResponse, roomResponse, mainRoomStatusResponse] = await Promise.all([
            axios.get(
          ...
          if (attendeeResponse.data.records.length > 0) {
            ...
            console.log('Data sent to MainRoomStatus:', mainRoomStatusResponse.data);
          } else {
    ...
    <div className="min-h-screen bg-image flex flex-col items-center justify-center">
      <div className="bg-white p-12 my-8 rounded-lg shadow-2xl transform hover:scale-105 transition-transform duration-300">
        ...
        <input
          ...
          className="w-full px-6 py-4 rounded-lg border-4 border-sky-500 focus:outline-none focus:ring-4 focus:ring-sky-300 focus:border-transparent text-2xl text-gray-800 placeholder-gray-400"
        />
        {loading && (
